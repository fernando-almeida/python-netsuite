...
class GovernanceModel(object):
	...
	MANDATORY_ROOT_KEYS = [SCHEDULES_KEY, RECORD_LIMITS_KEY, REQUEST_LIMITS_KEY]
	...
	def get_operation_category(self, operation_name):
		...
		iter = ifilter(lambda rule: rule["regex"].match(operation_name), 
	...
	def get_operation_constraints(self, operation_name):
		...
		operation_category = self.get_operation_category(operation_name)
		if not operation_category:
			logger.debug("No category found for operation {0} so it has no constraints".format(operation_name))
			return None
		...
		operation_constraints.update(self.get_operation_category_constraints(operation_category, is_synchronous, schedule_id))
		operation_constraints.update(self.get_request_constraints(operation_name))
	...
	def get_operation_category_constraints(self, operation_category, is_synchronous = True, schedule_id = None):
		...
		if not "operations" in record_limits[synchronous_key][schedule_id]:
			raise Exception("Property \"operations\" is not on records_limits[\"{0}\"][\"{1}\"]".format(synchronous_key, schedule_id))

		operations = record_limits[synchronous_key][schedule_id]["operations"]

		if not operation_category in operations:
			raise Exception("Operation category {0} is not on record_limits[\"{1}\"][\"operations\"]".format(operation_category, synchronous_key))
	...
	def validate_config(cls, config):
		...
		if missing_mandatory_keys:
			raise Exception("The following mandatory keys (\"{0}\") are missing.".format('\",\"'.join(missing_mandatory_keys)))
